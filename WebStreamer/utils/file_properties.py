...
async def gen_link(m: Message,log_msg: Messages, from_channel: bool):
    ...
    file_id = get_media_file_unique_id(log_msg)
    ...
    Stream_Text=lang.stream_msg_text.format(file_name, file_size,file_id, stream_link, page_link)
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🖥 צפייה ישירה", url=page_link), InlineKeyboardButton("📥 הורד את הקובץ", url=stream_link)]])
